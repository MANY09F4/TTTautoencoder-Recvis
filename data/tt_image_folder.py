...
class ExtendedImageFolder_online_shuffle(datasets.ImageFolder):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        # Optionally print the original index
        if self.print_index and index % self.batch_size == 0:
            print(f"Datapoint {index // self.batch_size}: Shuffled index = {real_index}, Original index = {shuffled_real_index}")
