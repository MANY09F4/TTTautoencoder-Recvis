...
def create_imagenet_c_subset_with_empty_dirs(base_dir, output_dir, num_images=500):
    """
    Create a subset of ImageNet-C with the same selected images across all corruption types and severity levels.
    Also ensures that empty directories are created for classes with no selected images.
    """
    random.seed(23)  # Fix the seed for reproducibility
    ...
    # Collect all classes in the dataset
    all_classes = os.listdir(first_severity_dir)
    ...
    for corruption in corruptions:
        for severity in range(1, 6):  # Severity levels 1 to 5
            ...
            for cls in all_classes:
                # Ensure the class directory exists in the output, even if empty
                class_dir = os.path.join(output_corruption_dir, cls)
    ...
    print(f"Subset created at: {output_dir} with {num_images} images per corruption level, including empty class directories.")
...
if __name__ == "__main__":
    ...
    create_imagenet_c_subset_with_empty_dirs(base_dir, output_dir, num_images)
